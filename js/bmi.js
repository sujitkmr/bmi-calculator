...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 1. BMI Report PDF with chart + details
  if (downloadReport) {
    downloadReport.addEventListener("click", () => {
      ...
      doc.text(`BMI: ${lastBMI}`, 20, 56);
      doc.text(`Category: ${lastCategory}`, 20, 64);

      // BMI Chart (simple horizontal bar)
      doc.setFont("helvetica", "bold");
      doc.text("BMI Chart", 20, 80);
      doc.setFont("helvetica", "normal");
      ...
      const ranges = [
        { label: "Underweight", start: 0, end: 18.5, color: "#93c5fd" },
        { label: "Normal", start: 18.5, end: 24.9, color: "#86efac" },
        { label: "Overweight", start: 25, end: 29.9, color: "#fdba74" },
        { label: "Obese", start: 30, end: 40, color: "#fca5a5" }
      ];

      ranges.forEach(r => {
        const startX = 20 + (r.start / 40) * barWidth;
        const width = ((r.end - r.start) / 40) * barWidth;
        doc.setFillColor(r.color);
        doc.rect(startX, startY, width, barHeight, "F");
      });

      // Marker for user BMI
      const bmiX = 20 + (lastBMI / 40) * barWidth;
      doc.setDrawColor(0);
      doc.line(bmiX, startY - 2, bmiX, startY + barHeight + 2);
      doc.text(`Your BMI: ${lastBMI}`, bmiX, startY + 20, { align: "center" });

      // Suggestions
      doc.setFont("helvetica", "bold");
      doc.text("Health Suggestions", 20, 130);
      doc.setFont("helvetica", "normal");
      tips[lastCategory].list.forEach((t, i) => {
        doc.text(`- ${t}`, 25, 140 + i * 8);
      });
