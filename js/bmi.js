// bmi.js - BMI Calculator with Popup Modal, Left-Pane Report, and Insights Storage
document.addEventListener("DOMContentLoaded", () => {
  const bmiForm = document.getElementById("bmi-form");
  if (!bmiForm) return;

  // Left-pane basic report
  ...
  bmiForm.addEventListener("submit", (e) => {
    ...
    if (height > 0 && weight > 0) {
      ...
      // === Health Metrics ===
      const defaultWaist = gender === "male" ? 90 : 80;
      const defaultHip = gender === "male" ? 100 : 95;
      ...
      const bmiDataToStore = {
        name, age, dob, height, weight, gender,
        bmi, category, pbf, waistHipRatio, visceralFatLevel,
        fatToReduce, bmr, tdee, obesityDegree, currentDate: today
      };
      ...
      bmiData.push({
        bmi, category, age, gender, name, date: today, weight, bodyFat: pbf
      });
      localStorage.setItem("bmiData", JSON.stringify(bmiData));

      // Save insights
      let insightsData = JSON.parse(localStorage.getItem("bmiInsightsData")) || [];
      insightsData.push({ date: today, bmi, weight, bodyFat: pbf, category });
